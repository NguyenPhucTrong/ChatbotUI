...
export default function Profile() {
        ...
        const fetchUser = async () => {
            try {
                if (!idUser) {
                    ...
                    // Lấy `idUser` từ API và lưu vào state
                    setIdUser(user.IdUser);

                    console.log('Họ tên người dùng:', user.Fullname);
                } else {
                    ...
                    setCurrentPassword(user.Password); // Lưu mật khẩu hiện tại
                }
            } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            console.log('Giá trị Fullname trước khi gửi:', fullname);

            // Tạo payload với tất cả các trường
            const payload: any = {
                ...
                Role: role.trim(),
                Permission: 'default_permission', // Giá trị mặc định
                Password: currentPassword, // Gửi mật khẩu hiện tại
            };
            ...
            // Cập nhật localStorage với Username mới
            if (payload.Username && payload.Username !== currentUsername) {
                localStorage.setItem('username', payload.Username); // Cập nhật `localStorage` với Username mới
            }
            ...
            <div className="p-8 w-full max-w-3xl bg-white shadow-lg rounded-lg">
                ...
                <div className="flex flex-row items-center justify-center text-center mb-6 space-x-6">
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-2xl font-bold mt-2 text-center">{username}</p>
                    </div>
                </div>
                    ...
                    <div className="flex-1 ">
                        <p className="text-gray-600 my-5">
                            ...
                            <input
                                type="text"
                                value={username}
                                onChange={(e) => setUsername(e.target.value)} // Đảm bảo state được cập nhật khi input thay đổi

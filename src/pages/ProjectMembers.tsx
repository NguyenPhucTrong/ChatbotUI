...
import { getProjectMembers, addMemberToProject } from "../services/ProjectMembers";
...
const ProjectMembersManagement: React.FC = () => {
    ...
    const fetchMembers = async (projectId: number) => {
        try {
            ...
            console.log("API Response for getProjectMembers:", response); // Debug toàn bộ response

            const membersData = response.data; // Lấy dữ liệu từ response
            if (Array.isArray(membersData)) {
                // Nếu là mảng, xử lý bình thường
                setMembers(
                ...
                // Nếu là đối tượng, chuyển thành mảng
                setMembers([
                    {
                        id: membersData.IdUser,
                        fullname: membersData.Fullname,
                        email: membersData.Email,
                        phoneNumber: membersData.PhoneNumber,
                        role: membersData.UserRole,
                    },
                ]);
            } else {
    ...
    const handleAddUserToProject = async (userId: number) => {
        ...
        try {
            ...
            setUsers((prevUsers) =>
                prevUsers.map((user) =>
                    user.id === userId ? { ...user, projectId: selectedProjectId } : user
                )
            );
        } catch (error) {
    ...
    return (
        <div className="p-6 flex-1 max-w-[1493px] mx-auto overflow-hidden">
            <h1 className="text-3xl font-semibold my-6 text-gray-800">Project Members Management</h1>
                ...
                <div className="mb-6">
                    ...
                    <ul className="list-disc pl-6">
                        {members.length > 0 ? (
                            members.map((member) => (
                                <li key={member.id}>
                                    {member.fullname} ({member.email}{member.phoneNumber}) Role: {member.role}
                                </li>
                            ))
                        ) : (
                    ...
                    <table className="min-w-full bg-white border border-gray-300 rounded shadow">
                        <thead>
                            <tr className="bg-gray-300">
                                ...
                                <th className="px-6 py-4 text-left font-semibold text-gray-700 border-b">Phone Number</th>
